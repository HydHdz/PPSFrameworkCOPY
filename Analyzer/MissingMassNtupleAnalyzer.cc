...
void MissingMassNtupleAnalyzer::Loop(char * mode, char * year, char *era, char * jobid, char * outdir, char * physics, bool createProtonFile, bool randomFlag, bool zerobias, bool protonsfilter, bool createEventFile, bool optshort, bool notrigger, char* filenamepudata, char * filenamepumc, bool debug){

  bool puweight = false;

  if(filenamepumc && filenamepudata && strcmp(mode, "mc")==0){
    puweight = true;
    CheckPileupFile(filenamepudata);
    CheckPileupFile(filenamepumc);
    LumiWeights_ = new reweight::LumiReWeighting(filenamepumc,filenamepudata,"pileup","pileup");
  }
  ...
  if(strcmp(era, "A")==0){
    era_ = 0;
  }else if (strcmp(era, "B")==0){ 
    era_ = 1;
  }else if (strcmp(era, "C")==0){
    era_ = 2;
  }else if (strcmp(era, "D")==0){
    era_ = 3;
  }else if (strcmp(era, "E")==0){
    era_ = 4;
  }else if (strcmp(era, "F")==0){
    era_ = 5;
  }else{
    era_ = -1;
  }
  ...
  if(GetCheckComplete()){
    ttreeAnalysis.switchBjets = true;
    ttreeAnalysis.switchMuon = true;
    ttreeAnalysis.switchElectron = true;
  }else{
    ttreeAnalysis.switchBjets = true;
    ttreeAnalysis.switchMuon = false;
    ttreeAnalysis.switchElectron = false;
  }
  if(strcmp(year, "2017")==0) ttreeAnalysis.switchYear2017 = true;
  if(strcmp(year, "2018")==0) ttreeAnalysis.switchYear2018 = true;

  if(zerobias){
    ttreeAnalysis.switchZeroBias = true;
    ttreeAnalysis.switchMuon = false;
    ttreeAnalysis.switchElectron = false;
    ttreeAnalysis.switchBjets = false;
    ttreeAnalysis.switchDisplacedJet = false;
  }
  ...
  std::cout << "\t\t Pileup Reweight: " << puweight << std::endl;
  if(puweight){
    std::cout << "\t\t Pileup data file: " << filenamepudata << std::endl;
    std::cout << "\t\t Pileup mc file: " << filenamepumc << std::endl;
  }
  std::cout << "\t\t Automatic detected a reduced TTree: " << !GetCheckComplete() << std::endl;
  std::cout << "\t\t Automatic detected a MC TTree: " << GetCheckMC() << std::endl;
  ...
  for (Long64_t jentry=rangemin; jentry<rangemax;jentry++) {
    ...
    if(puweight){
      ttreeAnalysis.weight = LumiWeights_->weight(PUInterac);
    }else{
      ttreeAnalysis.weight = 1.;
    }
    ...
    if( (strcmp(mode, "mc")==0 && randomFlag) || (strcmp(mode, "data")==0 && randomFlag && zerobias) ){
	...
	ttreeAnalysis.flagrdm_rp210_Arm56=true;
	if(zerobias){
	...
	ttreeAnalysis.flagrdm_rp210_Arm45=true;
	if(zerobias){
	  ttreeAnalysis.nprotonRP210_sec45 = -2;
	}else{
	...
	ttreeAnalysis.flagrdm_rp210_Arm56=true;
	if(zerobias){
	  ttreeAnalysis.nprotonRP210_sec45 = -3;
	  ttreeAnalysis.nprotonRP210_sec56 = -3;
	}else{
	  ttreeAnalysis.nprotonRP210_sec45 = 1;
	  ttreeAnalysis.nprotonRP210_sec56 = 1;
	}
	...
	ttreeAnalysis.flagrdm_rp220_Arm56=true;
	if(zerobias){
	  ttreeAnalysis.nprotonRP220_sec56 = -4;
	}else{
	...
	ttreeAnalysis.flagrdm_rp220_Arm45=true;
	if(zerobias){
	  ttreeAnalysis.nprotonRP220_sec45 = -5;
	}else{
	...
	ttreeAnalysis.flagrdm_rp220_Arm56=true;
	if(zerobias){
	  ttreeAnalysis.nprotonRP220_sec45 = -6;
	  ttreeAnalysis.nprotonRP220_sec56 = -6;
	}else{
	  ttreeAnalysis.nprotonRP220_sec45 = 1;
	  ttreeAnalysis.nprotonRP220_sec56 = 1;
	}
	...
	ttreeAnalysis.flagrdm_multiArm56=true;
	if(zerobias){
	  ttreeAnalysis.nprotonMulti_sec56 = -7;
	}else{
	...
	ttreeAnalysis.flagrdm_multiArm45=true;
	if(zerobias){
	  ttreeAnalysis.nprotonMulti_sec45 = -8;
	}else{
	...
	ttreeAnalysis.flagrdm_multiArm56=true;
	if(zerobias){
	  ttreeAnalysis.nprotonMulti_sec45 = -9;
	  ttreeAnalysis.nprotonMulti_sec56 = -9;
	}else{
	  ttreeAnalysis.nprotonMulti_sec45 = 1;
	  ttreeAnalysis.nprotonMulti_sec56 = 1;
	}
    ...
    if(strcmp(mode, "mc")==0 || strcmp(mode, "data")==0){
      ...
      if(ttreeAnalysis.switchElectron||ttreeAnalysis.switchMuon){
	...
	if(zerobias){
	  ...
	  if((strcmp(physics, "muon")==0)){
	    if(strcmp(year, "2017")==0 || strcmp(year, "2018")==0){
	      if(trigger->at(0)==1) ttreeAnalysis.HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ = true; 
	      if(trigger->at(1)==1) ttreeAnalysis.HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass3p8 = true; 
	      if(trigger->at(2)==1) ttreeAnalysis.HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass8 = true; 
	      if(trigger->at(3)==1) ttreeAnalysis.HLT_IsoMu24_eta2p1 = true; 
	      if(trigger->at(4)==1) ttreeAnalysis.HLT_IsoMu24 = true; 
	      if(trigger->at(5)==1) ttreeAnalysis.HLT_IsoMu27 = true;
	      if(trigger->at(6)==1) ttreeAnalysis.HLT_Mu18_Mu9_SameSign = true; 
	      if(trigger->at(7)==1) ttreeAnalysis.HLT_Dimuon0_Upsilon_L1_4p5er2p0M = true; 
	      if(trigger->at(8)==1) ttreeAnalysis.HLT_BTagMu_AK8Jet300_Mu5 = true; 
	      if(trigger->at(9)==1) ttreeAnalysis.HLT_BTagMu_AK8DiJet170_Mu5 = true; 
	      if(trigger->at(10)==1) ttreeAnalysis.HLT_BTagMu_AK4Jet300_Mu5 = true; 
	      if(trigger->at(11)==1) ttreeAnalysis.HLT_BTagMu_AK4DiJet70_Mu5 = true;
	      if(trigger->at(12)==1) ttreeAnalysis.HLT_BTagMu_AK4DiJet40_Mu5 = true;
	      if(trigger->at(13)==1) ttreeAnalysis.HLT_BTagMu_AK4DiJet20_Mu5 = true;
	      if(trigger->at(14)==1) ttreeAnalysis.HLT_BTagMu_AK4DiJet170_Mu5 = true;
	      if(trigger->at(15)==1) ttreeAnalysis.HLT_BTagMu_AK8Jet300_Mu5_noalgo = true;
	      if(trigger->at(16)==1) ttreeAnalysis.HLT_BTagMu_AK8DiJet170_Mu5_noalgo = true;
	      if(trigger->at(17)==1) ttreeAnalysis.HLT_BTagMu_AK4Jet300_Mu5_noalgo = true;
	      if(trigger->at(18)==1) ttreeAnalysis.HLT_BTagMu_AK4DiJet70_Mu5_noalgo = true;
	      if(trigger->at(19)==1) ttreeAnalysis.HLT_BTagMu_AK4DiJet40_Mu5_noalgo = true;
	      if(trigger->at(20)==1) ttreeAnalysis.HLT_BTagMu_AK4DiJet20_Mu5_noalgo = true;
	      if(trigger->at(21)==1) ttreeAnalysis.HLT_BTagMu_AK4DiJet170_Mu5_noalgo = true;
	    ...
	    if(strcmp(year, "2017")==0||strcmp(year, "2018")==0){
	      ...
	      if(trigger->at(0)==1||trigger->at(1)==1||trigger->at(2)==1||trigger->at(3)==1){
      ...
      met.SetPtEtaPhiM(missEt, 0, missEt_phi, 0);met.SetPz(0);
      met.SetE(missEt);
	...
	ttreeAnalysis.Rmpf = 1.+(met.Px() * dilepton.Px() + met.Py() * dilepton.Py()) / (dilepton.Pt()*dilepton.Pt());
      ...
      else ttreeAnalysis.Rmpf = -1;

      if(jetsak4_pt->size()>0){
	jet1.SetPtEtaPhiE(jetsak4_pt->at(0), jetsak4_eta->at(0), jetsak4_phi->at(0), jetsak4_energy->at(0));
	if(ttreeAnalysis.switchElectron||ttreeAnalysis.switchMuon) ttreeAnalysis.JZBalance = jet1.Pt() - dilepton.Pt();
	else ttreeAnalysis.JZBalance = -1;
      }

      if(jetsak4_pt->size()>1){
	jet2.SetPtEtaPhiE(jetsak4_pt->at(1), jetsak4_eta->at(1), jetsak4_phi->at(1), jetsak4_energy->at(1));
	dijet = jet1 + jet2;
	if(ttreeAnalysis.switchElectron||ttreeAnalysis.switchMuon) ttreeAnalysis.JZBalance2 = (dijet).Pt() - dilepton.Pt();
	else ttreeAnalysis.JZBalance2 = -1;
      }

      // Invisible particle associated with dijet system
      if(jetsak4_pt->size()>1){ // new 
	X_dijetRP210 = p1RP210 + p2RP210 - dijet;
	X_dijetRP220 = p1RP220 + p2RP220 - dijet;
	X_dijetMulti = p1Multi + p2Multi - dijet;
      }

      if(ttreeAnalysis.switchMuon||ttreeAnalysis.switchElectron){
	...
	ttreeAnalysis.dijetMass = -999;
	ttreeAnalysis.dijetEta = -999;
	ttreeAnalysis.dijetPhi = -999;
	ttreeAnalysis.dijetPt = -999;
	ttreeAnalysis.dijetRapidity = -999;
	ttreeAnalysis.jetsystemMass = -999;
	ttreeAnalysis.jetsystemEta = -999;
	ttreeAnalysis.jetsystemPhi = -999;
	ttreeAnalysis.jetsystemPt = -999;
	ttreeAnalysis.jetsystemRapidity = -999;
      }
	...
	ttreeAnalysis.missingRapidityDileptonDijetMulti = X_dileptondijetMulti.Rapidity();
      ...
      double dphi = fabs(missEt_phi-dijet.Phi());
...
void MissingMassNtupleAnalyzer::CheckPileupFile(char* file){

  TFile f(file);
  if (f.IsZombie()) {
    std::cout << "Error opening PU file.\n" << std::endl;
    exit(-1);
  }

  if(!f.Get("pileup")){
    std::cout << "Pileup histogram name should be \"pileup\".\n" << std::endl;
    exit(-1);
  }
  f.Close();
...
int main(int argc, char * argv[])
{
  ...
  if(cmdOptionExists(argv, argv+argc, "--h")||cmdOptionExists(argv, argv+argc, "--help"))
  {
    ...
    std::cout << "\t\033[;33m --pudata filename.root (input pu histogram file, it must be used together with --pumc option). It applies pile-up reweighting." << std::endl;
    std::cout << "\t\033[;33m --pumc filename.root (input pu mc file, it must be used together with --pudata option). It applies pile-up reweighting." << std::endl;
    ...
    std::cout << "\t --pumc && --pudata (to apply the pileup reweighting for the MC)\n" << std::endl;
  ...
  char * filenamepudata = getCmdOption(argv, argv + argc, "--pudata");
  char * filenamepumc = getCmdOption(argv, argv + argc, "--pumc");
  ...
  if( (cmdOptionExists(argv, argv+argc, "--protonfile") || cmdOptionExists(argv, argv+argc, "--random"))&&(cmdOptionExists(argv, argv+argc, "--noppstagging")) ){
  ...
  if((cmdOptionExists(argv, argv+argc, "--pudata") || cmdOptionExists(argv, argv+argc, "--pumc")) && strcmp(mode, "data")==0){
    std::cout << "\n\t ---> [Pile-up Reweighting] can not run with --mode data! Please try --mode mc!\n" << std::endl;
    return 0;
  }

  if(((cmdOptionExists(argv, argv+argc, "--pudata") && !cmdOptionExists(argv, argv+argc, "--pumc"))) || ((!cmdOptionExists(argv, argv+argc, "--pudata") && cmdOptionExists(argv, argv+argc, "--pumc")))){
    std::cout << "\n\t ---> [Pile-up Reweighting] both options --pudata and --pumc must be used together!\n" << std::endl;
    return 0;
  }
  ...
  if (filename && mode && physics && year)
  {
    ...
    // Accessing Missing Mass Objects
    MissingMassNtupleAnalyzer m(tree);

    if(!m.GetCheckComplete() && (strcmp(physics, "muon")==0 || strcmp(physics, "electron")==0)){
      std::cout << "\n\t --> The fileinput is reduced (only jets branches). Thus, only --physics=bjet option can be used!\n" << std::endl;
      exit(EXIT_FAILURE);
    }

    if(m.GetCheckMC() && (strcmp(mode, "data")==0)){
      std::cout << "\n\t --> The input file is a TTree created from a MC dataset. Please, use the option --mode=mc instead of --mode=data!\n" << std::endl;
      exit(EXIT_FAILURE);
    }

    if(!m.GetCheckMC() && (strcmp(mode, "mc")==0)){
      std::cout << "\n\t --> The input file is a TTree created from the detector DATA. Please, use the option --mode=data instead of --mode=mc!\n" << std::endl;
      exit(EXIT_FAILURE);
    }

    m.Loop(mode, year, era, jobid, outdir, physics, createProtonFile, randomFlag, zerobias, protonsfilter, createEventFile, optshort, notrigger, filenamepudata, filenamepumc, debug);
