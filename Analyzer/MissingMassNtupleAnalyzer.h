...
class MissingMassNtupleAnalyzer {
  public :
    ...
    virtual void     Loop(char*, char*, char*, char*, char*, char*, bool, bool, bool, bool, bool, bool, bool, char*, char*, bool);
    ...
    void CheckPileupFile(char*);
    bool MCMatch(double, double, double, double, double, double);
    bool GetCheckComplete() const {return switchAll;}
    bool GetCheckMC() const {return switchMC;}
...
void MissingMassNtupleAnalyzer::Init(TTree *tree)
{
  ...
  for(int i = 0; i < list->GetEntries(); ++i) {
    ...
    if(bSearch.find("gen") != std::string::npos || bSearch.find("Gen") != std::string::npos){
      switchMC = true;
    }
    if(bSearch.find("nElectrons") != std::string::npos || bSearch.find("nMuons")!= std::string::npos){
      switchAll = true;
    }
  }
